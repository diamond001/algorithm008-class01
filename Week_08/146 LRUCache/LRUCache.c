...
typedef struct
{
    int capacity;
    int count;
    struct list_head dhead; /* double linked list */
...
LRUCache *lRUCacheCreate(int capacity)
{
    ...
    INIT_LIST_HEAD(&obj->dhead); /* 双向链表 */
    for (i = 0; i < capacity; i++)
    {
        INIT_HLIST_HEAD(&obj->hhead[i]); /* 每一个哈希地址对应一个双向链表，貌似java实现没有搞这么复杂啊 */
    }
    return obj;
}
...
{
    LRUNode *cache = NULL;
    int hash = key % obj->capacity; /* hash map */
    struct hlist_node *pos;
    hlist_for_each(pos, &obj->hhead[hash])  /* 根据计算出的哈希地址hash，遍历当前所计算出的hash地址所对应的双向链表 */
    {
        LRUNode *c = list_entry(pos, LRUNode, node); /* 获取LRUNode的指针 */
        if (c->key == key)  /* 如果当前哈希表里面存在相同的key值 */
        {
            list_move(&c->link, &obj->dhead); /* 则把该key值对应的节点移动到双向链队列的头部 */
            cache = c;
        }
    }

    if (cache == NULL) /* 如果当前哈希表中木有找到相同的key，则判断如果哈希表是否已满，若满，则移除队列的尾结点，否则插入到头结点之前 */
    {
        if (obj->count == obj->capacity)
        {
            cache = list_last_entry(&obj->dhead, LRUNode, link); /* why last?? */
            list_move(&cache->link, &obj->dhead);
